<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        {{-- ROLE SELECTION BLOCK (VISIBLE ONLY TO LOGGED-IN SUPER ADMIN) --}}
        @role('admin super user')
            <div class="mt-4">
                ...
                <select id="role" name="role" 
                    ...
                    <option value="admin secondary" {{ old('role') == 'admin secondary' ? 'selected' : '' }}>Admin Secondary</option>
                    <option value="normal end user" {{ old('role') == 'normal end user' ? 'selected' : '' }}>Normal End User</option>
                </select>
                
                <x-input-error :messages="$errors->get('role')" class="mt-2" />
            </div>
        @endrole
        {{-- END ROLE SELECTION BLOCK --}}
